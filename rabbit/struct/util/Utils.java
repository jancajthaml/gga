...
public final class Utils
{
    ...
    @SuppressWarnings({"unchecked"}) public static <T> T[] arrayOf( T first, T second, T ... more )
    {
    	_not_null_argument( more, "more" );
    	
        Class<?>  elementType  =  more.getClass().getComponentType();
        T[]       array        =  (T[]) Array.newInstance(elementType, more.length + 2);
                  array [ 0 ]  =  first;
                  array [ 1 ]  =  second;
        
        System.arraycopy( more, 0, array, 2, more.length);
        
        return array;
    }

    @SuppressWarnings({"unchecked"}) public static <T> T[] arrayOf( T[] first, T last )
    {
    	_not_null_argument( first, "first" );
    	
        Class<?>  elementType  =  first.getClass().getComponentType()                      ;
        T[]       array        =  (T[]) Array.newInstance( elementType, first.length + 1 ) ;
        
        System.arraycopy( first, 0, array, 0, first.length );
        
        array [ first . length ]  =  last;
