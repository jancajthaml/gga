...
import pal.api.signature.util._Array;
import pal.api.signature.util._Int;
import pal.api.signature.util._List;
import pal.api.signature.util._String;
...
public class GenericDirectedAcyclicGraph<T> implements Iterable<List<GenericDirectedAcyclicGraph<T>.Node>>
{
    ...
    public class Node
    {
		...
		public void addEdgeColor(int index, int color)
	...
	public void init(String ... vertexLabels)
	{
        ...
        List<_String> pairs = new ArrayList<_String>();
        
        for( int i = 0; i < vertexCount; i++ )
            pairs.add(new _String(vertexLabels[i], parentCounts[i], i));
	...
	public void updateLayer( List<GenericDirectedAcyclicGraph<T>.Node> layer, GenericDirectedAcyclicGraph.Direction direction )
	{
        ...
        for( int i = 0; i < layer.size(); i++ )
        {
            ...
            for( Node relative : relatives )
            {
                ...
                relativeInvariants.add( vertexCount + 1 + (( direction==Direction.UP ) ? (relative.colors.get(layerNode.index)) : (layerNode.colors.get(relative.index))) );
            }
